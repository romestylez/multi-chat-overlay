...
```bash
php -l /var/www/vhosts/multichat.romestylez.dev/httpdocs/youtube-proxy.php
...
Security headers must be configured at the outer web server so they also cover files served directly by nginx. The production deployment currently uses `Content-Security-Policy-Report-Only`, so it cannot block OBS while Twitch, Kick, YouTube, badges and all enabled emote providers are being verified. `connect-src 'self'` already permits the same-origin YouTube proxy; YouTube images are covered by `img-src https:`.

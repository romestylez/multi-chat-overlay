...
- Optional channel-specific emote IDs
...
The overlay always attempts to load the supported global emote sets. Channel-specific requests are made only when the required value is configured.

- `SEVENTV_USER_ID`: optional 7TV user ID for channel-specific emotes
- `BTTV_TWITCH_USER_ID`: optional Twitch user ID for channel-specific BetterTTV emotes
- `FFZ_CHANNEL`: optional FrankerFaceZ channel name

When `FFZ_CHANNEL` is empty, the configured Twitch channel is used. A failed optional emote request does not stop the remaining overlay services.
...
| `SEVENTV_USER_ID` | Optional channel-specific 7TV user ID | Empty |
| `BTTV_TWITCH_USER_ID` | Optional Twitch user ID for channel-specific BTTV emotes | Empty |
...
- Optional emote IDs do not control the platform connections.

...
- `HIDE_BADGES` hides every supported Twitch and Kick badge.
- `HIDE_SUB_BADGES` hides only Twitch subscriber and founder badges.
- `HIDE_GLOBAL_BADGE` hides only the global badge selected by a Twitch user.
- `HIDE_PLATFORM` hides the Twitch or Kick platform icon.
...
7TV and BetterTTV can be enabled or disabled independently in the editor. When enabled, their global emotes are loaded automatically.

For channel-specific emotes, the overlay reads the Twitch channel ID from IRC and uses it directly with the 7TV and BetterTTV APIs. No separate 7TV or BetterTTV ID has to be entered.

- `ENABLE_7TV`: load global and channel-specific 7TV emotes
- `ENABLE_BTTV`: load global and channel-specific BetterTTV emotes
- `FFZ_CHANNEL`: optional FrankerFaceZ channel name

When `FFZ_CHANNEL` is empty, the configured Twitch channel is used. A failed optional emote request does not stop the remaining overlay services or chat connections.
...
| `FFZ_CHANNEL` | Optional FFZ channel | Twitch channel |
...
| `HIDE_PLATFORM` | Hide Twitch and Kick platform icons | `false` |
...
The legacy `SEVENTV_USER_ID` and `BTTV_TWITCH_USER_ID` settings are still accepted as optional manual overrides by the overlay. They are no longer required or generated by the graphical editor.
...
Open the public Kick channel endpoint manually, find `chatroom.id` and enter that number in the editor. Do not use the top-level channel `id`.
...
- 7TV and BetterTTV can be disabled without affecting the Twitch connection.
